  ...
  try {
    if (
      ...
      const message = await client.messages.create({
        body: `Hi ${pickerName}, you have collected ${netAmount} ${unit.toLowerCase()} of ${product.toLowerCase()}. That means you've earned ${currency} ${+payment.toFixed(1)} for your next paycheck.`,
        from: "+16205914371",
        to: `+${pickerPhone}`,
      });
    }
  ...
  try {
    for (let index = 0; index < phoneSMS.length; index++) {
      if (
        ...
        const message = await client.messages.create({
          body: `Hi ${nameSMS[index]}, your payment for this payroll is ${currencySMS} ${+netAmountSMS[index].toFixed(1)} for a total collection of ${collectedAmountSMS[index]} ${unitSMS[index]} of ${productSMS}.`,
          from: "+16205914371",
          to: `+${phoneSMS[index]}`,
        });
      }
